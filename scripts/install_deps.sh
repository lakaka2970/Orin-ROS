...
case "$UBUNTU_VERSION" in
    ...
    22.04)
        ...
        ;;
    *)
        echo "⚠ 未识别的 Ubuntu 版本: $UBUNTU_VERSION"
...
if [ -f "$ROS2_SETUP" ]; then
    echo "  ✅ ROS2 $ROS2_DISTRO 已安装"
else
    if $WITH_ROS2; then
        echo "  📦 安装 ROS2 $ROS2_DISTRO ($ROS2_DESKTOP_PKG)..."
        ...
        if $DRY_RUN; then
            ...
            # 验证
            if [ -f "$ROS2_SETUP" ]; then
                echo "  ✅ ROS2 $ROS2_DISTRO 安装成功"
            else
                echo "  ⚠ 未找到 $ROS2_SETUP，但 apt 报告成功"
...
_verify_py() {
    ...
    if python3 -c "import $mod" 2>/dev/null; then
        ...
        echo "  ⚠ $label — source $ROS2_SETUP 后即可用"
    fi
}
...
if [ -f "$ROS2_SETUP" ]; then
    if bash -c "source $ROS2_SETUP 2>/dev/null && python3 -c 'import rclpy; import cv_bridge' 2>/dev/null"; then
        echo "  ✅ rclpy + cv_bridge (ROS2 $ROS2_DISTRO)"
    else
        echo "  ⚠ rclpy / cv_bridge 验证失败 — 请手动检查 ROS2 安装"
    fi
fi
