...
# -*- coding: utf-8 -*-
"""
FT 雷达信号处理节点 —— CUDA 加速实现 (R SP Cuda)
================================================================================
融合 ADC 数据和车辆数据，使用 CUDA 加速方式执行雷达信号处理（模拟实现），
解决速度模糊问题，输出检测目标列表。
与 R SP MIL Python 为并行/替代方案，提供更低的 SNR 阈值（更高灵敏度）。
...
发布话题：
  /ft/det_list_cu    PointCloud2    检测目标列表 (x, y, z, velocity, snr)
...
SNR_THRESHOLD  = 8.0         # 信噪比阈值 (dB) — CUDA 版使用更低阈值（更高灵敏度）
...
class RspCudaNode(Node):
    ...
    def __init__(self):
        ...
        # ---------- 发布者 ----------
        self.pub_det = self.create_publisher(PointCloud2, '/ft/det_list_cu', 10)
        ...
        self.get_logger().info(
            f'R SP Cuda 启动: {self.processing_fps:.0f} Hz, '
            f'SNR 阈值={self.snr_threshold} dB [GPU 模拟]')
    ...
    def _on_process(self):
        ...
        if len(x) == 0:
            self.get_logger().debug(
                f'[RSP-CUDA] 帧 #{self.frame_count}: 所有点低于 SNR 阈值，跳过')
            return
        ...
        self.get_logger().info(
            ...
            f'车速补偿 ego_vx={ego_vx:.1f} m/s → 发布 Det List (GPU)')
    ...
    def destroy_node(self):
        self.get_logger().info('R SP Cuda 已停止')
        super().destroy_node()
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info('R SP Cuda 收到中断信号，正在退出...')
    finally:
        node.destroy_node()
        rclpy.shutdown()
