...
// ============================================================================
// 启动时预生成 int16 噪声池, 每帧从池中随机切片.
// 可选: 若 data_dir 中存在 .bin 文件, 预加载至多 max_preload 帧用于循环回放.
//
    ...
    : RxNodeBase("adc_rx", "/adc/raw_data", 10, true)
  {
    ...
    if (!file_buffers_.empty()) {
      RCLCPP_INFO(get_logger(),
        "ADC Rx [analog]: %.0f Hz, %ux%ux%u, %.0f MB/帧, %zu 文件预加载 (%.0f MB RAM)",
  ...
  void init_data_source()
  {
    ...
    if (!paths_.empty()) {
      ...
      for (size_t i = 0; i < nload; ++i) {
        ...
        auto pos = f.tellg();
        if (pos <= 0) {
          RCLCPP_WARN(get_logger(), "空或无效文件, 跳过: %s", paths_[i].string().c_str());
          continue;
        }
